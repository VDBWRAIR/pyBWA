...
from bwa.bwa import BWA, BWAMem, BWAIndex
from bwa import seqio, bwa
...
class TestBWA( BaseBWA ):
    ...
    def setUpClass( self ):
        ...
        self.rops = {}
        for v, rop in enumerate( BWA.REQUIRED_OPTIONS ):
            self.rops[rop] = 'value'+str(v) 
...
'''
    ...
    def test_runbwa_defaultoutput( self ):
        ...
        bwa_path = self.mkbwa( '{0}'.format(output_text) )
    ...
    def test_compilebwaoptions( self ):
        ...
        with open( 'output' ) as fh:
            result_output = fh.read().strip()
        print "Expect: {0}".format(ops)
        print "Result: {0}".format(result_output)
        for op,val in ops.items():
            ...
            if val.lower() not in ('','none'):
                opval = '-{0}{1}'.format(op,uval) 
                assert opval in result_output, '{0} not in {1}'.format(opval, result_output)
            else:
                assert op not in result_output, '{0} should not be in {1}'.format(op, result_output)
...
class TestISIndexed(BaseBWA):
    ...
    def test_all_indexes_already_exist( self ):
        for ext in self.index_ext:
            create( '{0}.{1}'.format( self.fake_ref, ext ) )
    ...
    def test_reference_has_fai_file_only( self ):
        create( '{0}.{1}'.format( self.fake_ref, 'fai' ) )
        eq_( False, self._CII( self.fake_ref ) )
